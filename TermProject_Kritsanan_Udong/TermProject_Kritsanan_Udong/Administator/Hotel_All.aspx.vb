Public Class Hotel_All1
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Using Data As New DataOfTravelingDataContext
            ...
            Next
            newHTML.Append("<a class='nav-link text-success fw-blod' href='Hotel_Add.aspx'>+ เพิ่มข้อมูล</a>")
            Me.Literal_Hotel_NameTaB.Text = newHTML.ToString()
            ...
            For i = 1 To Convert.ToInt16(DataOfAll1.Count)
                ...
                newHTML.Append("<H3 class='text-center'>" & DataOfAllInside.First.Hotel_Name & "</H3>")
                newHTML.Append("<img src = '../allPicture/DataOfTraveling/Hotel_All/" & DataOfAllInside.First.Hotel_Image & "' class='d-block h-75 rounded-4 w-75 mx-auto py-3' style='width:400px;height:300px'/>")
                newHTML.Append("<H5 class='mx-4'>" & DataOfAllInside.First.Hotel_Details & "</H5>")
                newHTML.Append("<H4 class='mx-4 text-center'>ที่อยู่โรงแรม: " & DataOfAllInside.First.Hotel_Location & "</H4>")
                newHTML.Append("<H4 class='mx-4 text-center'>เบอร์โทรโรงแรม: " & DataOfAllInside.First.Hotel_PhoneNumber & "</H4>")
                newHTML.Append("<br />")
                newHTML.Append("<H5 class='mx-4 text-center'>ราคาต่อคืน: " & DataOfAllInside.First.Hotel_PriceOfDay & " บาท</H5>")
                ...
                newHTML.Append("</div>")
                ' Part of ADMIN '
                newHTML.Append("<div class='text-center my-5'><a href='Hotel_EDIT.aspx?ItemID=" & i & "'<Button class='mx-2 btn btn2 btn-success'>แก้ไขข้อมูล</button></a></div>")
