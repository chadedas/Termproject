<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="TravelingPlace_List.aspx.vb" Inherits="TermProject_Kritsanan_Udong.TravelingPlace_List" %>
...
<div class=" container marketing rounded-1 px-2" style="background-color:white">
    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataKeyNames="Travel_Id" DataSourceID="SqlDataSource1" CssClass="w-100 h-100 table col">
        <Columns>
            <asp:BoundField DataField="Travel_Id" HeaderText="ID" ReadOnly="True" SortExpression="Travel_Id" />
            <asp:BoundField DataField="Travel_Name" HeaderText="ชื่อสถานที่" SortExpression="Travel_Name" />
            <asp:BoundField DataField="Travel_Details" HeaderText="รายละเอียด" SortExpression="Travel_Details" />
            <asp:ButtonField ButtonType="Button" HeaderText="แก้ไขข้อมูล" Text="Button" CommandName="User_Edit1" />
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:Database1ConnectionString %>" SelectCommand="SELECT * FROM [Travel]"></asp:SqlDataSource>
    <div class="py-5 text-center"><asp:Button ID="Button2" runat="server" Text="เพิ่มสถานที่" CssClass="btn btn-primary mx-3 border-0 align-content-center mx-3 py-2"/><asp:Button ID="Button3" runat="server" Text="กราฟ" CssClass="btn btn-primary mx-3 border-0 align-content-center mx-auto py-2"/></div>
    <!-- /END THE FEATURETTES -->
